...
BEGIN
  ...
  WHERE user_id = NEW.user_id AND to_char(date, 'YYYY-MM') = v_month;
  ...
  IF v_pct >= 90 THEN
    ...
    IF v_supabase_url IS NOT NULL AND v_service_key IS NOT NULL THEN
      PERFORM extensions.http_post(
        ...
        body    := json_build_object('user_id', NEW.user_id, 'percentage', ROUND(v_pct),
                     'total_spent', ROUND(v_total, 2), 'budget', v_budget,
                     'category', NEW.category, 'category_total', NEW.amount)::text,
        headers := json_build_object('Content-Type', 'application/json',
                     'Authorization', 'Bearer ' || v_service_key)::jsonb
      );
    END IF;
  END IF;
